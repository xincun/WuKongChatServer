...
func (sm *ServiceMinio) UploadFile(filePath string, contentType string, copyFileWriter func(io.Writer) error) (map[string]interface{}, error) {
	...
	uploadUl, _ := url.Parse(sm.ctx.GetConfig().UploadURL)
	endpoint := fmt.Sprintf("%s", uploadUl.Host)
	accessKeyID := sm.ctx.GetConfig().MinioAccessKeyID
	secretAccessKey := sm.ctx.GetConfig().MinioSecretAccessKey
	useSSL := false
...
func (sm *ServiceMinio) DownloadURL(path string, filename string) (string, error) {
	vals := url.Values{}
	vals.Set("response-content-disposition", fmt.Sprintf("inline; filename=\"%s\"", filename))
	return fmt.Sprintf("%s%s?%s", sm.ctx.GetConfig().FileDownloadURL, path, vals.Encode()), nil
}
