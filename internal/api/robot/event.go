...
func (rb *Robot) saveRobotMessage(message *config.MessageResp, robotID string) {
	...
	messageUpdateJson := util.ToJson(&robotEvent{
		EventID: seq,
		Message: message,
		Expire:  time.Now().Add(rb.ctx.GetConfig().RobotMessageExpire).Unix(),
	})
	...
	err = rb.ctx.GetRedisConn().Expire(key, rb.ctx.GetConfig().RobotMessageExpire)
	if err != nil {
...
func (rb *Robot) messagesListen(messages []*config.MessageResp) {
	for _, message := range messages {
		...
		if contentMap != nil && contentMap["robot_id"] != nil {
			...
			if robotID != nil {
				if robotID == config.New().SystemUID {
