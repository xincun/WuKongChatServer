...
func (cn *Common) Route(r *wkhttp.WKHttp) {
	common := r.Group("/v1/common", r.AuthMiddleware(cn.ctx.Cache(), cn.ctx.GetConfig().TokenCachePrefix))
	{
...
func (cn *Common) appConfig(c *wkhttp.Context) {
	...
	if cn.ctx.GetConfig().ShortnoEditOff {
		shortnoEditOff = 1
	}
	...
	c.JSON(http.StatusOK, &appConfigResp{
		...
		WebURL:         cn.ctx.GetConfig().WebLoginURL,
