...
func (co *Conversation) Route(r *wkhttp.WKHttp) {
	...
	coversations := r.Group("/v1/coversations", r.AuthMiddleware(co.ctx.Cache(), co.ctx.GetConfig().TokenCachePrefix))
	{
	...
	// TODO: 这个里的接口后面移到 conversation的组里，因为单词拼错了 😭
	cnversation := r.Group("/v1/coversation", r.AuthMiddleware(co.ctx.Cache(), co.ctx.GetConfig().TokenCachePrefix))
	{
		cnversation.PUT("/clearUnread", co.clearConversationUnread)

	}

	conversation := r.Group("/v1/conversation", r.AuthMiddleware(co.ctx.Cache(), co.ctx.GetConfig().TokenCachePrefix))
	{
		// 离线的最近会话
	...
	conversations := r.Group("/v1/conversations", r.AuthMiddleware(co.ctx.Cache(), co.ctx.GetConfig().TokenCachePrefix))
	{
