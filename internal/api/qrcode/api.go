...
func (q *QRCode) Route(r *wkhttp.WKHttp) {
	// 获取二维码内的信息
	r.GET(q.ctx.GetConfig().QRCodeInfoURL, r.AuthMiddleware(q.ctx.Cache(), q.ctx.GetConfig().TokenCachePrefix), q.handleQRCodeInfo)
}
...
func (q *QRCode) handleQRCodeInfo(c *wkhttp.Context) {
	...
	uidAndName, err := q.ctx.Cache().Get(q.ctx.GetConfig().TokenCachePrefix + token)
	if err != nil {
...
func (q *QRCode) handleJoinGroup(loginUID string, qrCodeModel common.QRCodeModel) (interface{}, error) {
	...
	return NewHandleResult(ForwardH5, HandlerTypeWebView, map[string]interface{}{
		"url": fmt.Sprintf("%s/join_group.html?group_no=%s&auth_code=%s", q.ctx.GetConfig().H5BaseURL, groupNo, authCode),
	}), nil
}
