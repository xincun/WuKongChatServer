...
func TestFriendSureSearch(t *testing.T) {
	...
	err = u.ctx.Cache().SetAndExpire(u.ctx.GetConfig().FriendApplyTokenCachePrefix+token+testutil.UID, util.ToJson(map[string]interface{}{
		"from_uid": "222",
		"code":     vercode,
	}), u.ctx.GetConfig().FriendApplyExpire)
	assert.NoError(t, err)
