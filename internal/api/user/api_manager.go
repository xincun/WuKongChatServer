...
func (m *Manager) Route(r *wkhttp.WKHttp) {
	...
	auth := r.Group("/v1/manager", r.AuthMiddleware(m.ctx.Cache(), m.ctx.GetConfig().TokenCachePrefix))
	{
...
func (m *Manager) login(c *wkhttp.Context) {
	...
	if userInfo.Role != string(common.Admin) && userInfo.Role != string(common.SuperAdmin) {
		c.ResponseError(errors.New("登录账号未开通管理权限"))
		return
	}
	token := util.GenerUUID()
	// 将token设置到缓存
	err = m.ctx.Cache().SetAndExpire(m.ctx.GetConfig().TokenCachePrefix+token, fmt.Sprintf("%s@%s@%s", userInfo.UID, userInfo.Name, userInfo.Role), m.ctx.GetConfig().TokenExpire)
	if err != nil {
...
func (m *Manager) addFileHelperFriend(uid string) error {
	...
	isFriend, err := m.friendDB.IsFriend(uid, m.ctx.GetConfig().SystemFileUID)
	if err != nil {
		m.Error("查询用户关系失败")
		return err
	}
	if !isFriend {
		...
		err := m.friendDB.Insert(&FriendModel{
			UID:     uid,
			ToUID:   m.ctx.GetConfig().SystemFileUID,
			Version: version,
		})
		if err != nil {
...
func (m *Manager) addSystemFriend(uid string) error {
	...
	isFriend, err := m.friendDB.IsFriend(uid, m.ctx.GetConfig().SystemUID)
	if err != nil {
		m.Error("查询用户关系失败")
		return err
	}
	...
	if !isFriend {
		...
		err := m.friendDB.InsertTx(&FriendModel{
			UID:     uid,
			ToUID:   m.ctx.GetConfig().SystemUID,
			Version: version,
		}, tx)
		if err != nil {
