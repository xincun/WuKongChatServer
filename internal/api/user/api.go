...
import (
	...
	"github.com/WuKongIM/WuKongChatServer/internal/api/source"
	"github.com/opentracing/opentracing-go"
	"github.com/opentracing/opentracing-go/ext"

	commonapi "github.com/WuKongIM/WuKongChatServer/internal/api/base/common"
	"github.com/WuKongIM/WuKongChatServer/internal/api/base/event"
	common2 "github.com/WuKongIM/WuKongChatServer/internal/api/common"
	...
	"github.com/WuKongIM/WuKongChatServer/pkg/network"
	"github.com/WuKongIM/WuKongChatServer/pkg/util"
	"github.com/WuKongIM/WuKongChatServer/pkg/wkevent"
...
func (u *User) Route(r *wkhttp.WKHttp) {
	auth := r.Group("/v1", r.AuthMiddleware(u.ctx.Cache(), u.ctx.GetConfig().TokenCachePrefix))
	{
		// 根据uid查询用户信息
		auth.GET("/users/:uid", u.get)
		// 获取用户的会话信息
		// auth.GET("/users/:uid/conversation", u.userConversationInfoGet)

		//上传用户头像
		auth.POST("/users/:uid/avatar", u.uploadAvatar)
		// 更新用户设置
		auth.PUT("/users/:uid/setting", u.setting.userSettingUpdate)
	}

	user := r.Group("/v1/user", r.AuthMiddleware(u.ctx.Cache(), u.ctx.GetConfig().TokenCachePrefix))
	{
		// 注册用户设备
		user.POST("/device_token", u.registerUserDeviceToken)
		// 卸载用户设备
		user.DELETE("/device_token", u.unregisterUserDeviceToken)
		// 上传设备红点数量
		user.POST("/device_badge", u.registerUserDeviceBadge)
		// 授权登录
		user.GET("/grant_login", u.grantLogin)
		//修改用户信息
		user.PUT("/current", u.userUpdateWithField)
		// 我的二维码
		user.GET("/qrcode", u.qrcodeMy)
		// 更新我的设置
		user.PUT("/my/setting", u.userUpdateSetting)
		//添加黑名单
		user.POST("/blacklist/:uid", u.addBlacklist)
		//移除黑名单
		user.DELETE("/blacklist/:uid", u.removeBlacklist)
		//黑名单列表
		user.GET("/blacklists", u.blacklists)
		//设置聊天密码
		user.POST("/chatpwd", u.setChatPwd)
		//设置锁屏密码
		user.POST("/lockscreenpwd", u.setLockScreenPwd)
		// 设置多久后锁屏
		user.PUT("/lock_after_minute", u.lockScreenAfterMinuteSet)
		//关闭锁屏密码
		user.DELETE("/lockscreenpwd", u.closeLockScreenPwd)
		//客服列表
		user.GET("/customerservices", u.customerservices)
		// 注销用户
		user.DELETE("/destroy/:code", u.destroyAccount)
		//获取注销账号短信验证码
		user.POST("/sms/destroy", u.sendDestroyCode)
		// ---------- 登录设备管理 ----------
		// 用户登录设备
		user.GET("/devices", u.deviceList)
		// 删除登录设备
		user.DELETE("/devices/:device_id", u.deviceDelete)
		// 用户在线列表（我的设备和我的好友）
		user.GET("/online", u.onlineList)
		user.POST("/online", u.onlinelistWithUIDs)
		// 退出pc登录
		user.POST("/pc/quit", u.pcQuit)
		...
		// ---------- signal protocol ----------
		// 添加signal相关key 用户加密消息
		user.POST("/signal/keys", u.signalKeyAdd)
		// 获取某个频道的signal key
		user.POST("/signal/getkey", u.signalKeyGet)
	}
	v := r.Group("/v1")
	{
		//用户注册
		...
		//获取忘记密码验证码
		v.POST("/user/sms/forgetpwd", u.getForgetPwdSMS)
		//重置登录密码
		v.POST("/user/pwdforget", u.pwdforget)
		// 搜索用户
		v.GET("/user/search", u.search)

		// 用户头像
		v.GET("/users/:uid/avatar", u.UserAvatar)
		// 获取用户所在IM节点信息
		v.GET("/users/:uid/im", u.userIM)
		// 获取扫描用的登录uuid
		v.GET("/user/loginuuid", u.getLoginUUID)
		v.GET("/user/loginstatus", u.getloginStatus)
		//获取注册短信验证码
		v.POST("/user/sms/registercode", u.sendRegisterCode)
		// 通过认证码登录
		v.POST("/user/login_authcode/:auth_code", u.loginWithAuthCode)
		//发送登录设备验证验证码
		v.POST("/user/sms/login_check_phone", u.sendLoginCheckPhoneCode)
		//登录验证设备手机号
		v.POST("/user/login/check_phone", u.loginCheckPhone)
	}

	// 监听在线状态
	u.ctx.AddOnlineStatusListener(u.onlineService.listenOnlineStatus)
...
func (u *User) updateSystemUserToken() {
	_, err := u.ctx.UpdateIMToken(config.UpdateIMTokenReq{
		...
		Token:       "iPL1LJcPsQ@s@RtX#r9ZwxehW6$3^!f!",
	...
	_, err = u.ctx.UpdateIMToken(config.UpdateIMTokenReq{
		UID:         u.ctx.GetConfig().SystemFileUID,
		Token:       "iPL1LJcPsQ@s@RtX#r9ZwxehW6$3^!f!",
		DeviceFlag:  config.APP,
		DeviceLevel: config.DeviceLevelMaster,
	})
	...
	// 系统管理员
	_, err = u.ctx.UpdateIMToken(config.UpdateIMTokenReq{
		UID:         u.ctx.GetConfig().SystemAdminUID,
		Token:       "iPL1LJcPsQ@s@RtX#r9ZwxehW6$3^!f!",
		DeviceFlag:  config.APP,
		DeviceLevel: config.DeviceLevelMaster,
	})
...
func (u *User) UserAvatar(c *wkhttp.Context) {
	...
	if u.ctx.GetConfig().IsVisitor(uid) {
		...
		avatarBytes, err := ioutil.ReadFile("configs/assets/visitor.png")
		if err != nil {
	...
	if uid == u.ctx.GetConfig().SystemFileUID {
		c.Header("Content-Type", "image/jpeg")
		avatarBytes, err := ioutil.ReadFile("configs/assets/fileHelper.jpeg")
		if err != nil {
	...
	if userInfo.IsUploadAvatar == 1 {
		avatarID := crc32.ChecksumIEEE([]byte(uid)) % 100
		c.Redirect(http.StatusMovedPermanently, fmt.Sprintf("%s/avatar/%d/%s.png?v=%s", u.ctx.GetConfig().FileDownloadURL, avatarID, uid, v))
	} else {
		//访问默认头像
		avatarID := crc32.ChecksumIEEE([]byte(uid)) % 963
		c.Redirect(http.StatusMovedPermanently, fmt.Sprintf("%s/avatar/default/test (%d).jpg", u.ctx.GetConfig().FileDownloadURL, avatarID))
	}
...
func (u *User) uploadAvatar(c *wkhttp.Context) {
	...
	avatarID := crc32.ChecksumIEEE([]byte(loginUID)) % 100
...
func (u *User) userIM(c *wkhttp.Context) {
	uid := c.Param("uid")
	resp, err := network.Get(fmt.Sprintf("%s/route?uid=%s", u.ctx.GetConfig().IMURL, uid), nil, nil)
	if err != nil {
...
func (u *User) qrcodeMy(c *wkhttp.Context) {
	...
	c.Response(gin.H{
		"data": fmt.Sprintf("%s/%s", u.ctx.GetConfig().BaseURL, path),
	})
}
...
func (u *User) userUpdateWithField(c *wkhttp.Context) {
	...
	for key, value := range reqMap {
		...
		if key == "short_no" {
			if u.ctx.GetConfig().ShortnoEditOff {
		...
		if key == "name" {
			...
			err = u.ctx.Cache().Set(u.ctx.GetConfig().TokenCachePrefix+c.GetHeader("token"), fmt.Sprintf("%s@%s@%s", loginUID, value, c.GetLoginRole()))
			if err != nil {
...
func (u *User) get(c *wkhttp.Context) {
	...
	if u.ctx.GetConfig().IsVisitor(uid) { // 访客频道
		...
		u.ctx.Server.GetRoute().HandleContext(c)
		return
	}
...
func (u *User) wxLogin(c *wkhttp.Context) {
	...
	accessTokenResp, err := network.Get("https://api.weixin.qq.com/sns/oauth2/access_token", map[string]string{
		"appid":      u.ctx.GetConfig().WXAppID,
		"secret":     u.ctx.GetConfig().WXSecret,
		"code":       req.Code,
	...
	if userInfo != nil {
		...
		u.checkLoginInfo(userInfo, config.DeviceFlag(req.Flag), req.Device, loginSpanCtx, c)
	} else {
		// 创建用户
		uid := util.GenerUUID()
		...
		// 下载微信用户头像并上传
		if headimgurl != "" {
			file, _ := u.fileService.DownloadImage(headimgurl)
			if file != nil {
				avatarID := crc32.ChecksumIEEE([]byte(uid)) % 100
...
// 验证登录用户信息
func (u *User) checkLoginInfo(userInfo *Model, flag config.DeviceFlag, device *deviceReq, loginSpanCtx context.Context, c *wkhttp.Context) {
	if userInfo.Status == int(common.UserDisable) {
		c.ResponseError(errors.New("该用户已被禁用"))
		return
	}
	...
	if flag == 0 && userInfo.DeviceLock == 1 {
		...
		if device != nil {
			...
			if err != nil {
				...
				c.ResponseError(errors.New("查询是否存在的设备失败"))
				return
			}
			if existDevice {
		...
		if !existDevice {
			err := u.ctx.GetRedisConn().SetAndExpire(fmt.Sprintf("%s%s", u.ctx.GetConfig().LoginDeviceCachePrefix, userInfo.UID), util.ToJson(device), u.ctx.GetConfig().LoginDeviceCacheExpire)
			if err != nil {
				u.Error("缓存登录设备失败！", zap.Error(err))
				c.ResponseError(errors.New("缓存登录设备失败！"))
				return
			}
	...
	oldToken, err := u.ctx.Cache().Get(fmt.Sprintf("%s%d%s", u.ctx.GetConfig().UIDTokenCachePrefix, flag, userInfo.UID))
	if err != nil {
	...
	if flag == config.APP {
		if oldToken != "" {
			err = u.ctx.Cache().Delete(u.ctx.GetConfig().TokenCachePrefix + oldToken)
			if err != nil {
	...
	err = u.ctx.Cache().SetAndExpire(u.ctx.GetConfig().TokenCachePrefix+token, fmt.Sprintf("%s@%s@%s", userInfo.UID, userInfo.Name, userInfo.Role), u.ctx.GetConfig().TokenExpire)
	if err != nil {
	...
	err = u.ctx.Cache().SetAndExpire(fmt.Sprintf("%s%d%s", u.ctx.GetConfig().UIDTokenCachePrefix, flag, userInfo.UID), token, u.ctx.GetConfig().TokenExpire)
	if err != nil {
...
func (u *User) sentWelcomeMsg(publicIP, uid string) {
	...
	err = u.ctx.SendMessage(&config.MsgSendReq{
		FromUID:     u.ctx.GetConfig().SystemUID,
		ChannelID:   uid,
		ChannelType: common.ChannelTypePerson.Uint8(),
...
func (u *User) register(c *wkhttp.Context) {
	...
	if u.ctx.GetConfig().RegisterOff {
...
func (u *User) getLoginUUID(c *wkhttp.Context) {
	...
	c.JSON(http.StatusOK, gin.H{
		"uuid":   uuid,
		"qrcode": fmt.Sprintf("%s/%s", u.ctx.GetConfig().BaseURL, strings.ReplaceAll(u.ctx.GetConfig().QRCodeInfoURL, ":code", uuid)),
	})
}
...
func (u *User) loginWithAuthCode(c *wkhttp.Context) {
	...
	// 获取老的token
	token, err := u.ctx.Cache().Get(fmt.Sprintf("%s%d%s", u.ctx.GetConfig().UIDTokenCachePrefix, flag, scaner))
	if err != nil {
		u.Error("获取旧token错误", zap.Error(err))
		c.ResponseError(errors.New("获取旧token错误"))
		return
	...
	// 将token设置到缓存
	err = u.ctx.Cache().SetAndExpire(u.ctx.GetConfig().TokenCachePrefix+token, fmt.Sprintf("%s@%s", userModel.UID, userModel.Name), u.ctx.GetConfig().TokenExpire)
	if err != nil {
		u.Error("设置token缓存失败！", zap.Error(err))
		c.ResponseError(errors.New("设置token缓存失败！"))
		return
	...
	err = u.ctx.Cache().SetAndExpire(fmt.Sprintf("%s%d%s", u.ctx.GetConfig().UIDTokenCachePrefix, flag, userModel.UID), token, u.ctx.GetConfig().TokenExpire)
	if err != nil {
		u.Error("设置uidtoken缓存失败！", zap.Error(err))
		c.ResponseError(errors.New("设置uidtoken缓存失败！"))
		return
...
func (u *User) sendRegisterCode(c *wkhttp.Context) {
	...
	if u.ctx.GetConfig().RegisterOnlyChina {
...
func (u *User) loginCheckPhone(c *wkhttp.Context) {
	...
	loginDeviceJsonStr, err := u.ctx.GetRedisConn().GetString(fmt.Sprintf("%s%s", u.ctx.GetConfig().LoginDeviceCachePrefix, req.UID))
	if err != nil {
	...
	token := util.GenerUUID()
	// 将token设置到缓存
	err = u.ctx.Cache().SetAndExpire(u.ctx.GetConfig().TokenCachePrefix+token, fmt.Sprintf("%s@%s", userInfo.UID, userInfo.Name), u.ctx.GetConfig().TokenExpire)
	if err != nil {
		u.Error("设置token缓存失败！", zap.Error(err))
		c.ResponseError(errors.New("设置token缓存失败！"))
		return
...
func (u *User) addFileHelperFriend(uid string) error {
	...
	isFriend, err := u.friendDB.IsFriend(uid, u.ctx.GetConfig().SystemFileUID)
	if err != nil {
		u.Error("查询用户关系失败")
		return err
	}
	if !isFriend {
		...
		err := u.friendDB.Insert(&FriendModel{
			UID:     uid,
			ToUID:   u.ctx.GetConfig().SystemFileUID,
			Version: version,
		})
		if err != nil {
...
func (u *User) addSystemFriend(uid string) error {
	...
	isFriend, err := u.friendDB.IsFriend(uid, u.ctx.GetConfig().SystemUID)
	if err != nil {
		u.Error("查询用户关系失败")
		return err
	}
	...
	if !isFriend {
		...
		err := u.friendDB.InsertTx(&FriendModel{
			UID:     uid,
			ToUID:   u.ctx.GetConfig().SystemUID,
			Version: version,
		}, tx)
		if err != nil {
...
func (u *User) createUser(registerSpanCtx context.Context, createUser *createUserModel, c *wkhttp.Context) {
	...
	if u.ctx.GetConfig().ShortnoNumOn {
	...
	if err != nil {
		...
		c.ResponseError(errors.New("注册用户失败"))
		return
	}
	if createUser.Device != nil {
		...
		if err != nil {
			tx.Rollback()
			c.ResponseError(errors.New("添加用户设备信息失败"))
			return
		}
	}
	...
	if err != nil {
		tx.Rollback()
		c.ResponseError(errors.New("添加注册用户和系统账号为好友关系失败"))
		return
	}
	err = u.addFileHelperFriend(createUser.UID)
	if err != nil {
		tx.Rollback()
		c.ResponseError(errors.New("添加注册用户和文件助手为好友关系失败"))
		return
	}
	//发送用户注册事件
	...
	token := util.GenerUUID()
	// 将token设置到缓存
	err = u.ctx.Cache().SetAndExpire(u.ctx.GetConfig().TokenCachePrefix+token, fmt.Sprintf("%s@%s@%s", userModel.UID, userModel.Name, userModel.Role), u.ctx.GetConfig().TokenExpire)
	if err != nil {
		u.Error("设置token缓存失败！", zap.Error(err))
		c.ResponseError(errors.New("设置token缓存失败！"))
		return
	...
	c.Response(newLoginUserDetailResp(userModel, token, u.ctx))
}
