...
func (g *Group) Route(r *wkhttp.WKHttp) {
	group := r.Group("/v1/group", r.AuthMiddleware(g.ctx.Cache(), g.ctx.GetConfig().TokenCachePrefix))
	{
		group.POST("/create", g.groupCreate)
		group.GET("/my", g.list)                            //我保存的群
		group.GET("/forbidden_times", g.forbiddenTimesList) // 获取禁言时常列表
	}
	groups := r.Group("/v1/groups", r.AuthMiddleware(g.ctx.Cache(), g.ctx.GetConfig().TokenCachePrefix))
	{
...
func (g *Group) avatarGet(c *wkhttp.Context) {
	...
	v := c.Query("v")
	//是否为系统群
	if groupNo == g.ctx.GetConfig().SystemGroupID {
		c.Header("Content-Type", "image/jpeg")
		avatarBytes, err := ioutil.ReadFile("configs/assets/g_avatar.jpeg")
		if err != nil {
	...
	c.Redirect(http.StatusMovedPermanently, fmt.Sprintf("%s/%s?v=%s", g.ctx.GetConfig().FileDownloadURL, path, v))
}
...
func (g *Group) groupQRCode(c *wkhttp.Context) {
	...
	c.Response(gin.H{
		"day":    7,
		"qrcode": fmt.Sprintf("%s/%s", g.ctx.GetConfig().BaseURL, strings.ReplaceAll(g.ctx.GetConfig().QRCodeInfoURL, ":code", uuid)),
