...
func (g *Group) handleRegisterUserEvent(data []byte, commit config.EventCommit) {
	...
	//查询群聊是否存在
	groupModel, err := g.db.QueryWithGroupNo(g.ctx.GetConfig().SystemGroupID)
	if err != nil {
		g.Error("查询群详情失败")
		commit(err)
		return
	...
	if groupModel == nil {
		...
		err = g.db.InsertTx(&Model{
			GroupNo: g.ctx.GetConfig().SystemGroupID,
			Name:    g.ctx.GetConfig().SystemGroupName,
			Creator: g.ctx.GetConfig().SystemUID,
		...
		realMemberUids = append(realMemberUids, g.ctx.GetConfig().SystemUID)
		// 创建IM频道
		err = g.ctx.IMCreateOrUpdateChannel(&config.ChannelCreateReq{
			ChannelID:   g.ctx.GetConfig().SystemGroupID,
	...
	err = g.addMembers(realMemberUids, g.ctx.GetConfig().SystemGroupID, g.ctx.GetConfig().SystemUID, "系统账号")
	if err != nil {
